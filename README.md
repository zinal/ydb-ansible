...
| `ydb_archive` |  YDB server binary package path (typically `.tar.xz`, `.tar.gz` is also supported) |
| `ydb_version` | YDB version label used to define the installation directory path for the particular version of YDB binary package. Typically depends on the `ydb_archive` value |
...
| `ydb_config_storage` | Optional name of dedicated static nodes configuration file. When unset, static config is generated from `ydb_config`. |
...
1. Store the Vault password in a local file and restrict permissions:
   ```bash
   printf '%s\n' 'your-vault-password' > .vault-password
...
1. Repeat the previous two steps as necessary to create more databases and deploy more YDB dynamic nodes.
...
1. Ensure that the configuration variable `ydb_config` in the `group_vars/all` file points to the desired YDB server configuration file.
1. Ensure that the configuration variable `ydb_tls_dir` points to the directory containing the desired TLS key and certificate files for all the nodes within the YDB cluster.
...
1. `acl` package is installed on Debian-like systems
...
1. kernel/sysctl network settings are applied for YDB hosts
1. optional TCP congestion control kernel module is configured and loaded when `ydb_congestion_module` is set
...
The provided `ansible.cfg` uses `vault_password_file = .vault-password`, so Ansible reads the Vault password from that file.
You can override this behavior by setting `ANSIBLE_VAULT_PASSWORD_FILE` (for example in CI), or by adjusting `ansible.cfg`.
